...
def main(args):
    ...
    if re.match(r"TMBA", sqlfile):
        ...
        with open(csvfile, "r") as fob:
            ...
            for N, row in enumerate(csv.reader(fob)):
                ...
                dst = "{} {}".format(
                    time.strftime("%Y-%m-%d", d), time.strftime("%H:%M:%S", t)
                )
                ...
                if N == flen - 1:
                    loc.extend(
                        [
                            dst,
                            1
                            if re.match(r"Recovered", row[1])
                            else 7
                            if re.match(r"Deployed", row[1])
                            else 6,
                            row[1],
                        ]
                    )
    ...
    sql = [
        ...
        r"SELECT `inst_id` INTO @con_inst_id FROM `ControllersInfo` WHERE `sn` = '{}' AND `type_id` = {}; INSERT INTO `InstrumentHist` (`inst_id`, `dt`, `txt`) VALUES".format(
            sn, tp
        ),
    ]
    for N, li in enumerate(data):
        if N == len(data) - 1:
            ...
            sql[1] += r" (@con_inst_id,'{}','{}'),".format(*li)
    sql.append(
        r"INSERT INTO {}InstrumentLocation.History (`inst_id`,`dt`,`label_id`,`txt`) VALUES (@con_inst_id,'{}',{},'{}');".format(
            pfx, *loc
        )
    )
    sql.append(str())
