    ...
    with open("TMBAIridium.csv", "w") as fob:
        ...
        for r in row_iter:
            ...
            if any([not str(c.value).strip().isdigit() for c in r[0:5]]):
                ...
                continue
            csv_row = [
                (((c.value is not None) and str(c.value).strip()) or str())
                for c in r[0:6]
            ]
            try:
                csvobj.writerow(csv_row)
            ...
            with open("TMBA{}.csv".format(sn), "a") as fobj:
                ...
                for c in row:
                    ...
                    if c.value is not None and isinstance(c.value, datetime):
                        ...
                        csv_row.append("")
                        csv_row.append(
                            str(wb[sht].cell(row=c.row, column=c.column + 1).value) + " mA"
                        )
    ...
    with open("TMBAInventory.csv", "w") as fob:
        ...
        for r in row_iter:
            if r[0].value is None:
                ...
                break
            csv_row = [
                (((c.value is not None) and str(c.value).strip()) or str()) for c in r
            ]
            try:
                csvobj.writerow(csv_row)
            except UnicodeEncodeError as uee:
...
def make_ocs_hist(sns):
    for sn in sns:
        ...
        with open("OCS{}.csv".format(sn), "w") as fobj:
            header = ["dt", "comment"]
            csvobj = csv.DictWriter(
                fobj, fieldnames=header, quotechar='"', quoting=csv.QUOTE_MINIMAL
            )
